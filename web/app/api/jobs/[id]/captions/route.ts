...
/** WebVTT captions for a done job (Phase 4 — Deepgram narration). */
export async function GET(_req: NextRequest, { params }: Params) {
  ...
  try {
    ...
    const vttPath = job.voice?.captions;
    if (!vttPath || typeof vttPath !== "string") {
      return NextResponse.json({ error: "no captions for this job" }, { status: 404 });
    }
